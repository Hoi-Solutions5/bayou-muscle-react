...
const product = {
  ...
  flavours: ['Chocolate Fudge', 'Vanilla Ice Cream', 'Strawberry', 'Salted Caramel', 'Unflavoured'],
...
const relatedProducts = [
  { id: 1, name: 'CPT: Conjugated Linoleic Acid Supplement', price: '$9.00', stars: 4, img: 'https://images.unsplash.com/photo-1595348020949-87cdfbb44174?w=400&q=80', category: 'Weight Management' },
  { id: 2, name: 'Denzour Micronised Creatine 100g', price: '$9.95', stars: 4, img: 'https://images.unsplash.com/photo-1579722821273-0f6c7d44362f?w=400&q=80', category: 'Performance' },
  { id: 4, name: 'L-Carnitine: Amino Acid Supplement', price: '$9.00', stars: 4, img: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=400&q=80', category: 'Amino Acids' },
  { id: 5, name: 'Ultra Ripped: Thermogenic Fat Burner', price: '$11.90', stars: 5, img: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&q=80', category: 'Weight Management' },
];

const reviews = [
  { name: 'Sarah M.', date: 'March 2025', rating: 5, title: 'Best protein I\'ve tried', text: 'Mixes perfectly with no clumps, tastes amazing and I\'ve noticed real improvements in my recovery time. Will definitely re-order.' },
  { name: 'James K.', date: 'February 2025', rating: 5, title: 'Clean and effective', text: 'I\'m picky about macros and this hits the mark. Low fat, low carb, high protein. The chocolate flavour is indulgent without being too sweet.' },
  { name: 'Priya T.', date: 'January 2025', rating: 4, title: 'Great quality protein', text: 'Really impressed with the quality. Slightly pricey but you get what you pay for. Packaging is sturdy and the scoop is proper-sized.' },
];
  ...
  return (
    <div style={{ display: 'flex', gap: 2 }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} style={{ color: i < count ? 'var(--color-primary)' : '#ddd', fontSize: size }}>★</span>
      ))}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="pd-breadcrumb">
        ...
        <a href="#" onClick={e => { e.preventDefault(); onNavigate('shop'); }}>Supplements</a>
        <span className="pd-bc-sep">›</span>
      ...
      <section className="pd-main">
        ...
        <div className="pd-gallery">
          ...
          <div className="pd-main-img">
            {product.badge && <span className="product-badge pd-badge">{product.badge}</span>}
            <img src={product.thumbs[activeThumb].replace('w=200', 'w=800')} alt={product.name} />
          </div>
        </div>
        ...
        <div className="pd-info">
          ...
          <div className="pd-price-row">
            ...
            {product.badge && <span className="pd-save-badge">Save {product.badge}</span>}
          </div>
          ...
          {/* Buy Now */}
          <button className="pd-buy-btn">Buy Now — ${(product.price * qty).toFixed(2)}</button>
        ...
        <div className="pd-tab-content">
          {activeTab === 'Description' && (
            <div className="pd-tab-desc">
              <div className="pd-tab-desc-text">
                ...
                <p>Our cold-filtration technology removes lactose, fat and denatured proteins while retaining the native protein structure. The result is a cleaner, purer protein that your body can absorb rapidly post-workout.</p>
                <h4>Directions</h4>
                <p>Mix 1 scoop (30g) with 250-300ml cold water or milk. Consume within 30 minutes of training. Can also be used as a high-protein snack between meals.</p>
                <h4>Allergen Information</h4>
                <p>Contains milk (whey). Manufactured in a facility that also processes soy, eggs, and tree nuts. Suitable for vegetarians. Not suitable for vegans.</p>
              </div>
              <div className="pd-tab-desc-img">
                <img src="https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=600&q=80" alt="Protein supplement in use" />
              </div>
            </div>
          )}
          ...
          {activeTab === 'Nutrition' && (
            <div className="pd-tab-nutrition">
              ...
              <p className="pd-nutrition-note">*Percent Daily Values are based on a 2,000 calorie diet.</p>
            </div>
          )}
            ...
            <div className="pd-tab-reviews">
              <div className="pd-reviews-summary">
                <div className="pd-reviews-score">
                  ...
                  <span className="pd-score-sub">{product.reviewCount} verified reviews</span>
                </div>
                  ...
                  {[5, 4, 3, 2, 1].map(n => (
                    <div key={n} className="pd-rating-bar-row">
                      ...
                      <div className="pd-rating-bar">
                        <div className="pd-rating-fill" style={{ width: n === 5 ? '72%' : n === 4 ? '20%' : n === 3 ? '6%' : '2%' }} />
                      </div>
                      <span className="pd-bar-pct">{n === 5 ? '72%' : n === 4 ? '20%' : n === 3 ? '6%' : n === 2 ? '1%' : '1%'}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="pd-related">
        <div className="pd-related-inner">
          <h2 className="section-heading" style={{ marginBottom: '40px' }}>You May Also Like</h2>
          <div className="pd-related-grid">
            {relatedProducts.map(p => (
              <div key={p.id} className="product-card" onClick={() => window.__navigate && window.__navigate('product')}>
